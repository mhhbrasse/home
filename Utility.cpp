...
void createTextureFromFK6(TRender& myRenderer, char* in_filename, char* out_filename)
{
	...
	while (!feof(f))
	{
		...
		printf ( "r is %f\n", r );
		myRenderer.drawCircle( (int) RA, 1023 + (int) DEC, 1, r*255, r*255, r*225 );
...
void createTextureFromStarCatalogDat(TRender& myRenderer, char* in_filename, char* out_filename)
{
	...
	for (int i =0;i<1628;i++)	
	{
		...
		// [-1.46/5.03]
		float r = MIN(10.0f*pow(10,-V/2.5),1.0f);		
		r=sqrt(r);
		...
		if (V<0.0)
		{
			printf("V negative\n");
			for (int j=3; j<4; j++)
				myRenderer.drawCircle( (int) RA, 1023 + (int) DEC, j, r*255, r*255, r*225 );
		}
